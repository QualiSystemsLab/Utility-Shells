...
class AppLicensePoolDriver (ResourceDriverInterface):
    ...
    def get_inventory(self, context):
        ...
        resource = AppLicensePool.create_from_context(context)
        requested_ports = int(resource.pool_limit)

        for i in range(1, requested_ports + 1):
            if i < 10:
                port_num = f"0{i}"
            else:
                port_num = str(i)
            port = ResourcePort("license" + port_num)
            resource.add_sub_resource(port_num, port)
